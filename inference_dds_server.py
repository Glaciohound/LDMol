...
def running_under_torchrun() -> bool:
    # torchrun sets these
    return ("RANK" in os.environ and "WORLD_SIZE" in os.environ)

def init_distributed_if_needed(backend="nccl"):
    if running_under_torchrun():
        # Use the env:// rendezvous provided by torchrun
...
def load_model(args, LDMOL_DIR):
    # Decide distributed vs single GPU
    is_ddp, rank = init_distributed_if_needed()
    ...
    ae_config = {
        'bert_config_decoder': LDMOL_DIR / './config_decoder.json',
        'bert_config_encoder': LDMOL_DIR / './config_encoder.json',
        'embed_dim': 256,
    }
    tokenizer = regexTokenizer(vocab_path=LDMOL_DIR / './vocab_bpe_300_sc.txt', max_len=127)#newtkn
...
@torch.no_grad()
def edit_smiles(args, model_bundle, input_smiles, source_text, target_text):
    model = model_bundle['model']
    ae_model = model_bundle['ae_model']
    text_encoder = model_bundle['text_encoder']
    text_tokenizer = model_bundle['text_tokenizer']
    device = model_bundle['device']
    is_ddp = model_bundle['is_ddp']
